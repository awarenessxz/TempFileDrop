...
const BucketConsole = () => {
    ...
    useEffect(() => {
        if (userToken) {
            const requests = userToken.buckets.map(bucket => axios.get(`${Data.api_endpoints.storagesvc}/${bucket}`));
            Promise.all(requests)
                .then(results => {
                    ...
                    results.forEach(response => {
                        const bucketFileSystem: FileSystemNode = response.data;
                        rootDir.children.push(bucketFileSystem);
                    });
    ...
    const handleDelete = (parent: FileSystemNode, childIdx: number, node: FileSystemNode) => {
        axios.delete(`${Data.api_endpoints.storagesvc}/${node.storageBucket}/${node.storageId}`)
            .then(res => {
                if (res.status === 200) {
                                ...
                                <Table aria-label="simple table">
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            <TableCell/>
                                            <TableCell align="center">StorageId</TableCell>
                                    ...
                                    <TableBody>
                                        {currentDir.children.length > 0 && currentDir.children.map((fileSystemNode, idx) => (
                                            <TableRow key={fileSystemNode.storageId}>
